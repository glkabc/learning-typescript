...
type StrToUnion<T extends string> = T extends `${infer A}${infer B}` ? A | StrToUnion<B> : ''

type AllCombinations<S extends string, U extends string = StrToUnion<S>, R extends string = U> = 
    [U] extends [never] ? '' : U extends R ? `${U | ''}${AllCombinations<never, Exclude<R, U>>}` : never
